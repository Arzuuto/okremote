...
func (uc *UsersCreate) SetFistName(s string) *UsersCreate {
	uc.mutation.SetFistName(s)
	return uc
}
...
func (uc *UsersCreate) AddAdvertisementIDs(ids ...int) *UsersCreate {
	uc.mutation.AddAdvertisementIDs(ids...)
	return uc
}

// AddAdvertisement adds the "advertisement" edges to the Advertisement entity.
func (uc *UsersCreate) AddAdvertisement(a ...*Advertisement) *UsersCreate {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
	return uc.AddAdvertisementIDs(ids...)
}
...
func (uc *UsersCreate) check() error {
	if _, ok := uc.mutation.FistName(); !ok {
		return &ValidationError{Name: "FistName", err: errors.New(`ent: missing required field "Users.FistName"`)}
	}
	if v, ok := uc.mutation.FistName(); ok {
...
func (uc *UsersCreate) createSpec() (*Users, *sqlgraph.CreateSpec) {
	...
	if value, ok := uc.mutation.FistName(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: users.FieldFistName,
		})
		_node.FistName = value
	}
	...
	if nodes := uc.mutation.AdvertisementIDs(); len(nodes) > 0 {
