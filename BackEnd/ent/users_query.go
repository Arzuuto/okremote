...
type UsersQuery struct {
	...
	// eager-loading edges.
	withAdvertisement *AdvertisementQuery
...
// QueryAdvertisement chains the current query on the "advertisement" edge.
func (uq *UsersQuery) QueryAdvertisement() *AdvertisementQuery {
	query := &AdvertisementQuery{config: uq.config}
...
func (uq *UsersQuery) Clone() *UsersQuery {
	...
	return &UsersQuery{
		...
		withAdvertisement: uq.withAdvertisement.Clone(),
...
// WithAdvertisement tells the query-builder to eager-load the nodes that are connected to
// the "advertisement" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UsersQuery) WithAdvertisement(opts ...func(*AdvertisementQuery)) *UsersQuery {
	query := &AdvertisementQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withAdvertisement = query
	return uq
}
...
func (uq *UsersQuery) sqlAll(ctx context.Context) ([]*Users, error) {
	...
	if query := uq.withAdvertisement; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Users)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Advertisement = []*Advertisement{}
		}
		query.withFKs = true
		query.Where(predicate.Advertisement(func(s *sql.Selector) {
			s.Where(sql.InValues(users.AdvertisementColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.users_advertisement
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "users_advertisement" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "users_advertisement" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Advertisement = append(node.Edges.Advertisement, n)
		}
	}
